jQuery(function($){ // use jQuery code inside this to avoid "$ is not defined" error
	$('.medyc_loadmore').click(function(){
		...
		var button = $(this),
			...
			data = {
				'action': 'loadmore',
				'query': medyc_loadmore_params.posts, // that's how we get params from wp_localize_script() function
				'page' : medyc_loadmore_params.current_page,
		...
		$.ajax({ // you can also use $.post here
			url : medyc_loadmore_params.ajaxurl, // AJAX handler
			data : data,
			type : 'POST',
			...
			success : function( data ){
				if( data ) { 
					...
					medyc_loadmore_params.current_page++;
	...
	if( $('.medyc_loadmore').hasClass('infinite-box') ) {
		canBeLoaded = true;
	...
	$(window).scroll(function(){
		var data = {
			'action': 'loadmore',
			'query': medyc_loadmore_params.posts,
			'page' : medyc_loadmore_params.current_page,
			'layoutPost' : layoutPost,
			'postsNumber' : postsNumber,
			'postsOrder' : postsOrder,
		...
		if( $(document).scrollTop() > ( $(document).height() - bottomOffset ) && canBeLoaded == true ){
			$.ajax({
				url : medyc_loadmore_params.ajaxurl,
				data:data,
				type:'POST',
				beforeSend: function( xhr ){
